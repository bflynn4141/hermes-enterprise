...
|---|---|
| The full Postgres schema: 37 tables, forced row-level security, three roles, the grant matrix, the append-only audit, the derived views | Executing an effect. `POST /w/:ws/effects/:id/execute` answers `unavailable` in words in production, and always will here. Development and staging answer `simulated` instead: an invented reference and timeline under its own status, labelled simulated everywhere ([docs/EFFECT-SIMULATION.md](docs/EFFECT-SIMULATION.md)) |
