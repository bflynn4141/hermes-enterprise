# Handoff for Astra: provision accounts, keys and settings for Hermes Enterprise
...
3. **Stop and ask Brian** before: paying for anything (Workers Paid plan, Neon paid tier, WorkOS custom domain), accepting a vendor's terms for a new account, deleting anything, or when a page does not look like the step describes. Do not improvise around an unexpected screen.
...
## Phase 0. Decisions Brian must confirm before anything permanent is created

Ask Brian for these in one message and wait:

| Decision | Why it is permanent | Default if Brian says "default" |
|---|---|---|
| Data residency: EU or default | R2 bucket jurisdiction and the Neon region cannot be changed after creation | default (US) |
| Hostnames | `ALLOWED_ORIGINS`, WorkOS redirect URIs and Workers custom domains all derive from them | `staging.<domain>` and `app.<domain>` on a zone already in Brian's Cloudflare account |
...
2. Create two branches from `main`: `staging` and `production`. (Or two projects if Brian prefers full isolation; the runbook accepts either.)
3. For each branch's compute: **Settings → Compute → Autosuspend: off** (this is "scale-to-zero off"; it is only available on paid plans, so this may require Brian's OK to upgrade). Size 0.5 CU is enough; note the connection limit shown (209 at 0.5 CU).
...
3. **Authentication settings** (Authentication tab): Email + password or Magic Auth on (Brian's choice; Magic Auth recommended), **Email verification on**, **Public sign-up off**, **JIT provisioning off** (found under Organizations or Authentication depending on the current dashboard), **Bot detection on**.
...
7. **Custom email provider**: Authentication → Email → provider Cloudflare (requires a Cloudflare email sending setup; if Brian has not set one up, leave WorkOS default sending and note it).
...
## Phase D. Values Astra generates locally
...
If Brian wants them in 1Password, create items named `Hermes staging secrets` and `Hermes production secrets` and paste them there through the 1Password app, not through chat.
...
pnpm --filter @hermes/worker exec wrangler secret put SENTRY_DSN --env $E     # only if Brian has a Sentry project; otherwise skip
```
...
All must be green (as of the handoff: 680 + 317 + 56 + 39 unit/db tests, 34+ live scenarios). If something fails here, stop and report; do not proceed to deploy.

## Phase G. GitHub

1. Create a **private** repository `hermes-enterprise` under Brian's GitHub account (ask which account). Add the remote and push `main`:
   ```sh
   git remote add origin git@github.com:<account>/hermes-enterprise.git
   git push -u origin main
   ```
2. Settings → Environments: create `staging` and `production`. On `production`, add Brian as a **required reviewer**.
...
3. In a browser: open `https://staging.<domain>/`, sign in through WorkOS (Brian's email; a WorkOS invitation or the create-workspace flow), create the workspace `Nous`.
4. **Provider key**: Settings → Provider keys → Add → provider **OpenRouter** → Brian pastes his rotated OpenRouter key himself (Astra does not handle it) → **Add and verify** (requires an Admin session signed in within the last five minutes) → the row reads `Verified · N models synced` → **Sync models** refreshes the list. The chat's model menu then lists every OpenRouter model; pick one that supports tools.
...
6. Record the Stop latency: with `HERMES_SPIKE_KEY` set to a key Brian provides in the terminal session only, run `pnpm --filter @hermes/worker spike` and paste the printed number into RUNBOOK §3.
...
Production follows the same steps only after Brian approves the `deploy-production.yml` run as the required reviewer.

## Report template (send to Brian when done or blocked)
