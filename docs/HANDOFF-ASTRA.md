...
4. **Provider key**: Settings → Provider keys → Add → provider **OpenRouter** (available once the OpenRouter change has landed on `main`; if the dropdown lacks it, stop and report) → Brian pastes his rotated OpenRouter key himself (Astra does not handle it) → Verify → status `verified` → Sync models. The chat's model menu then lists every OpenRouter model; pick one that supports tools.
