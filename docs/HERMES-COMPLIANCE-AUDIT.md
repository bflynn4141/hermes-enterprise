...
| Memory, skill, delegation, MCP and background features | compliant | — | The managed profile removes the general skill catalog and auto-loads only authenticated, agent-assigned plugin skills. `skill_view` can read only an assigned package and no linked file; skill listing/mutation, memory extraction/profile, nudges, background review/title generation, MCP, tool search, and delegation remain disabled (`runtime/hermes/start.py`, `runtime/hermes/enterprise_bridge/__init__.py`). Native session transcripts intentionally remain enabled. | Keep the agent-scoped manifest, exact-skill hook, generated-config assertion and native auto-load probe mandatory at every pin change. |
