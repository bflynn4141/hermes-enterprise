...
| Memory, skill, delegation, MCP and background features | demo-gated | P2 | The managed profile removes the general skill catalog and auto-loads only authenticated, agent-assigned plugin skills. Skill mutation, memory, nudges, background review, tool search and delegation remain disabled. MCP is empty by default; an explicit config can add at most eight stdio servers with per-server tool allowlists and scoped env references. AgentCash is pinned and further limited to StableEnrich/StableSocial with a $0.20 per-call cap, but third-party MCP code still needs a process sandbox and cumulative spend accounting before production. | Keep production MCP off until the hosted sandbox, cumulative API budget, audit capture and evidence-ingestion contract are verified. |
