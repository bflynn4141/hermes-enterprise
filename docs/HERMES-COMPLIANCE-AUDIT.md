...
| Memory, skill, delegation, MCP and background features | demo-gated | P2 | The managed profile removes the general skill catalog and auto-loads only authenticated, agent-assigned plugin skills. Skill mutation, memory, nudges, background review, tool search and delegation remain disabled. MCP is empty by default; an explicit config can add at most eight stdio servers with per-server tool allowlists and scoped env references. An invitee profile can expose only AgentCash `fetch`, and only for the exact StableEnrich People Search request leased by the Worker at a `$0.15` cap. Balance, discovery, schema inspection, StableSocial and alternate fetches are denied. Third-party MCP code still needs a hosted process sandbox and wallet reconciliation before broader production use. | Keep general production MCP off until the hosted sandbox, wallet reconciliation, audit capture and evidence-ingestion contract are verified. |
