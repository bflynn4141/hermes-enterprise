...
- Demo applications, invoices and agreements are useful for the demo flow when the UI labels the workspace as fictional/demo data. Their presence in seeds is not proof of a production fallback.
- Existing `unavailable` effect receipts are the correct fail-closed behavior until real bank/signature/mail adapters exist.
- Partner-program, admissions and Nous-specific copy is acceptable only inside the labeled demo scenario. Generic tenant onboarding and default agent configuration must use workspace-owned content.
