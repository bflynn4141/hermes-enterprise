// The client's half of "OpenRouter is the only provider" (decision R12).
//
...
describe('the Add-a-key dialog', () => {
  it('offers OpenRouter and nothing else', () => {
    expect(PROVIDER_CHOICES.map((choice) => choice.id)).toEqual(['openrouter']);
  ...
  it('says what a key unlocks, in the words the empty states use', () => {
    expect(EMPTY.providerKeys).toBe('Add your OpenRouter key to enable models');
    expect(EMPTY.noKey).toBe('Add your OpenRouter key in Settings to start');
    expect(EMPTY.keyNotAllowed).toContain('only OpenRouter keys');
  });
...
describe('the mock bundle', () => {
  ...
  it('is a fixture of the product, so it serves only OpenRouter rows', async () => {
    const mock = createMockBackend();
    const catalog = (await (await mock.fetchImpl(`/w/${mock.workspaceId}/catalog`)).json()) as {
      models: { provider: string }[];
    };
    expect(catalog.models.length).toBeGreaterThan(0);
    for (const row of catalog.models) expect(row.provider).toBe('openrouter');
    ...
    for (const key of keys.keys) expect(key.provider).toBe('openrouter');
  });
});
