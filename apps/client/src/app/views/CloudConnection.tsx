...
export function CloudConnection({ status, available, busy, error, onConnect }: CloudConnectionProps) {
  ...
  const presentation = cloudConnectionPresentation(status, available);
  ...
  return (
    <section className="cloud-connection" aria-labelledby={headingId} data-available={available ? 'true' : 'false'}>
