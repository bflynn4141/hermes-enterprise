...
function NoteEditor({ note, draftKey, latest, onRebase, onSave, onCancel }: { note: ContextNote | null; draftKey: string; latest?: ContextNote; onRebase: (note: ContextNote) => void; onSave: (title: string, text: string) => Promise<void>; onCancel: () => void }) {
  const [title, setTitle] = useState(noteDrafts.get(draftKey)?.title ?? note?.title ?? '');
  const [text, setText] = useState(noteDrafts.get(draftKey)?.text ?? note?.text ?? '');
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState('');
  const stale = !!note && latest?.revision !== note.revision;
  useEffect(() => { noteDrafts.set(draftKey, { note: note ?? 'new', title, text }); }, [draftKey, note, title, text]);
  useEffect(() => { const guard = (event: BeforeUnloadEvent) => event.preventDefault(); window.addEventListener('beforeunload', guard); return () => window.removeEventListener('beforeunload', guard); }, []);
  return <form className="agent-settings-editor" aria-label={note ? 'Edit note' : 'Add note'} onSubmit={(event) => {
    event.preventDefault();
    if (busy || stale || !title.trim() || !text.trim()) return;
    setBusy(true); setError('');
    void onSave(title.trim(), text.trim()).catch((caught: unknown) => {
      setError(['stale_revision', 'context_revision_conflict'].includes((caught as { reason?: string }).reason ?? '') ? 'This note changed elsewhere. Compare the current version below; your draft is kept.' : 'Could not save this note. Your text is still here. Try again.');
    ...
    {stale && <div><p className="meta">{latest ? 'Current saved version:' : 'This note was removed elsewhere. Copy your draft before cancelling.'}</p>{latest && <><p className="agent-settings-copy">{latest.title}\n{latest.text}</p><Button onClick={() => { onRebase(latest); setError(''); }}>I’ve reviewed the current note</Button></>}</div>}
    <div className="agent-settings-actions"><Button primary type="submit" disabled={busy || stale || !title.trim() || !text.trim()}>{busy ? 'Saving…' : 'Save note'}</Button><Button disabled={busy} onClick={onCancel}>Cancel</Button></div>
  </form>;
}
...
function ContextContents() {
  ...
  const [editor, setEditor] = useState<ContextNote | 'new' | null>(noteDrafts.get(draftKey)?.note ?? null);
  ...
  const refresh = useCallback(async () => {
    ...
    if (mounted.current && request === generation.current) { setNotes(notePage.items); setFiles(filePage.items); setLoading(false); }
  ...
  const closeEditor = () => { noteDrafts.delete(draftKey); setEditor(null); addButton.current?.focus(); };
  const beginEditor = (note: ContextNote | 'new') => { noteDrafts.delete(draftKey); setEditor(note); };
  ...
  const attach = (file: AttachmentDetail) => {
    if (!state.activeSessionId || !canAttach || !file.sha256 || file.extraction_status !== 'ready') return;
  ...
  const canAttach = !!session && session.agentId === agentId && !session.pendingTurn && !['working', 'waiting'].includes(session.run?.status ?? '') && session.draft.attachments.length < 5;
  return <div className="scroll"><div className="app-body agent-settings">
    <AgentHead /><AgentTabsRow value="context" />
    <div className="agent-settings-heading"><div><h2 className="display-28">What {agentName(state)} knows</h2><p className="meta">Program facts and source material.</p></div>{admin && <button ref={addButton} type="button" disabled={editor !== null} className="btn primary" onClick={() => { beginEditor('new'); setStatus(''); }}>+ Add context</button>}</div>
    ...
    {status && <div className="agent-settings-status" role="status">{status}</div>}
    {editor && <NoteEditor key={editor === 'new' ? 'new' : editor.id} note={editor === 'new' ? null : editor} draftKey={draftKey} latest={editor === 'new' || loading ? undefined : notes.find((note) => note.id === editor.id)} onRebase={setEditor} onCancel={closeEditor} onSave={async (title, text) => {
      if (!agentId) return;
      try {
        ...
        generation.current++; setNotes((current) => [saved, ...current.filter((note) => note.id !== saved.id)]); closeEditor(); setStatus('Note saved for future replies.');
    ...
    <section aria-label="Sources">
      ...
      {loading ? <Skeleton rows={2} /> : files.length === 0 ? <EmptyState icon="context" title="No sources yet" detail="Add a PDF, Markdown or text file up to 20 MB." /> : files.map((file) => <div className="agent-settings-row" key={file.id}>
        ...
        <div className="agent-settings-actions"><Button link onClick={() => setOpen(file)}>Open →</Button>{file.extraction_status === 'ready' && state.capabilities.turnAttachments && <Button disabled={!canAttach || session!.draft.attachments.some((item) => item.id === file.id)} onClick={() => attach(file)}>{session?.draft.attachments.some((item) => item.id === file.id) ? 'Selected' : 'Use in conversation'}</Button>}</div>
      </div>)}
    </section>
    ...
    <Dialog open={!!remove} title="Remove this note?" onClose={() => { if (!busy) setRemove(null); }} actions={<><Button disabled={busy} onClick={() => setRemove(null)}>Cancel</Button><Button disabled={busy} onClick={() => {
      ...
      setBusy(true);
      void adapter.rest.deleteContextNote(workspaceId, agentId, remove.id, remove.revision).then(() => { generation.current++; setNotes((current) => current.filter((note) => note.id !== remove.id)); setRemove(null); setStatus('Note removed from future replies.'); }).catch(() => { setError('Could not remove this note. It may have changed elsewhere.'); setRemove(null); void refresh().catch(() => undefined); }).finally(() => setBusy(false));
    ...
    <Dialog open={!!open} title={open?.name ?? 'Source'} onClose={() => setOpen(null)} actions={<><Button onClick={() => setOpen(null)}>Close</Button>{admin && <Button disabled={busy} onClick={() => {
      ...
      void adapter.rest.deleteUpload(workspaceId, 'agent_file', open.id).then(() => { generation.current++; setFiles((current) => current.filter((file) => file.id !== open.id)); setOpen(null); adapter.invalidateList(LIST_KEYS.agentFiles); setStatus('Source removed. Existing run snapshots are unchanged.'); }).catch(() => setError('Could not remove source. Try again.')).finally(() => setBusy(false));
