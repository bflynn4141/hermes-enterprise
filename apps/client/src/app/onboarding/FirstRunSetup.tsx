...
const STEP_LABELS: Record<FirstRunStep, string> = {
  intro: 'Meet Iris', criteria: 'Criteria', boundaries: 'Review', ready: 'Inbox',
};
      ...
      <div className="first-run-transcript" role="log" aria-live="polite">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div key={`${state.step}:${state.editingReviewers}:${irisStatus}`} className="first-run-current"
            ...
            {state.step === 'intro' && <Intro onContinue={() => onAction({ type: 'intro/continue' })} />}
            {state.step === 'criteria' && <Criteria state={state} onAction={onAction} />}
            {state.step === 'boundaries' && <Boundaries state={state} error={setupError} onAction={onAction} />}
            {state.step === 'ready' && <Ready status={irisStatus} onOpenInbox={onOpenInbox} onBack={() => onAction({ type: 'step/back' })} />}
          </motion.div>
        </AnimatePresence>
      </div>
    </section>
...
function Intro({ onContinue }: { onContinue: () => void }) {
  return <>
    <IrisPrompt>
      <p>Your organization has assigned you Iris. Let’s configure your first Partner Program workflow.</p>
...
function Criteria({ state, onAction }: { state: FirstRunState; onAction: (action: FirstRunAction) => void }) {
  return <>
    ...
    <form className="first-run-custom" onSubmit={(event) => { event.preventDefault(); onAction({ type: 'criteria/confirm' }); }}>
      ...
      <p className="first-run-gap"><Icon name="info" size={15} />You can refine industries, stages, locations, signals, and exclusions with Iris from Inbox.</p>
...
function Ready({ status, onOpenInbox, onBack }: { status: IrisReadyStatus; onOpenInbox?: () => void; onBack: () => void }) {
  const ready = status === 'ready' || status === 'idle';
  return <>
    <IrisPrompt>{ready
      ? <p>Your working agreement is saved. Iris is ready, and your first tasks are waiting in Inbox.</p>
      : status === 'retrying'
        ? <p>Iris is taking longer than expected. Your answers are saved and setup is retrying safely.</p>
        : <p>Your working agreement is saved. Getting Iris ready.</p>}
    </IrisPrompt>
    {!ready ? <div className="first-run-sample-progress" role="status" aria-live="polite"><span className="first-run-sample-label"><span className="first-run-live-dot" />Getting Iris ready</span></div> : null}
...
export function FirstRunWorkingAgreement({ state, agreement: supplied }: { state: FirstRunState; agreement?: WorkingAgreement }) {
  ...
  return <aside className="first-run-agreement" aria-label="Working agreement">
    <div className="first-run-agreement-heading"><div><Glass name="agreement" size={32} /><span><h2>Working agreement</h2><p>Partner Program Iris</p></span></div><span className={agreement.readyForWork ? 'is-ready' : ''}>{agreement.readyForWork ? 'Saved' : 'Draft'}</span></div>
    <p className="first-run-mode"><Icon name="loop" size={16} /><span><strong>Work until review</strong>Iris gathers evidence, then stops where a person owns the decision.</span></p>
