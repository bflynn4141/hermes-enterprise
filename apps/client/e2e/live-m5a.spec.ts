...
async function runTurn(page: Page, workspaceId: string, title: string): Promise<string> {
  ...
  const turn = await page.request.post(`/w/${workspaceId}/sessions/${sessionId}/turns`, {
    data: { text: 'Screen the applicant.', client_turn_id: randomUUID(), attachments: [], mode: 'work', model_id: 'deepseek-flash', effort: null },
    headers: { origin: ORIGIN },
  });
...
test('M1 · the trace detail shows the run\'s steps, its tool call and the arguments', async ({ browser }) => {
  ...
  // that called one.
  const app = pane(page);
  await expect(app.getByRole('heading', { name: /work · deepseek-flash/ })).toBeVisible({ timeout: 15_000 });
...
test('M3 · answering the destination from the Context tab writes it and unparks the run', async ({ browser }) => {
  ...
  psql(`
    ...
    INSERT INTO sessions (id, workspace_id, title, mode, model_id, owner_id)
      VALUES (${q(sessionId)}, ${q(fixture.workspaceId)}, 'M3 waiting', 'work', 'deepseek-flash', ${q(fixture.adminId)});
    INSERT INTO runs (id, workspace_id, session_id, status, waiting_for, waiting_label, model_id, client_turn_id, mode)
      VALUES (${q(runId)}, ${q(fixture.workspaceId)}, ${q(sessionId)}, 'waiting', 'destination', 'Feedback destination', 'deepseek-flash', ${q(randomUUID())}, 'work');
...
async function sweepEmptyStates(page: Page, seat: 'admin' | 'member'): Promise<void> {
  ...
  // Not an empty state: the catalog is seeded platform-wide, so the rows are
  // there and every one of them says *why* it cannot be chosen. That is the
  // honest shape — "no models" would be wrong, and a silent list of disabled
  // rows would be worse.
  await expect(app.getByText('No verified deepseek key')).toBeVisible();
