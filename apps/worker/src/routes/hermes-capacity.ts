...
import { HermesCloudClient } from '../hermes-cloud/client.js';
...
/**
 * Register existing, already-paid capacity only after both the Cloud control
 * plane and Enterprise connector prove it is healthy. This route never creates
 * or funds an instance.
 */
export async function registerHermesCapacity(c: Context<{ Bindings: Env }>): Promise<Response> {
  ...
  let capabilities;
  try {
    const management = new HermesCloudClient(c.env);
    const runtime = new HermesClient(url.toString(), input.control_secret, undefined, 'dashboard_connector');
    [cloud, readiness, capabilities] = await Promise.all([
      management.getAgent(input.cloud_agent_id),
  ...
  if (cloud.health !== 'HEALTHY' || !capabilities.durableIdempotency || readiness.workspaceId !== verified.workspaceId ||
  ...
  const result = await inWorkspace(c, async (work) => {
    ...
    const inserted = await work.tx.query<{ id: string; created_at: Date }>(
      `INSERT INTO hermes_cloud_capacity
         (id, workspace_id, cloud_agent_id, instance_name, dashboard_url, connector_url, state,
       ...
       VALUES ($1,$2,$3,$4,$5,$6,'available',$7,$8,$9,$10,$11,$12,true,true,true,now(),now())
       ON CONFLICT DO NOTHING
       RETURNING id, created_at`,
      [capacityId, work.workspaceId, input.cloud_agent_id, input.instance_name, cloud.dashboardUrl,
       url.toString(), Buffer.from(envelope.ciphertext), Buffer.from(envelope.iv),
