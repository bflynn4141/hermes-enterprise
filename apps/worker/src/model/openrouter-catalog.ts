// Turning OpenRouter's `GET /api/v1/models` into catalog rows.
//
...
// `normaliseModels` is the whole judgement, `syncOpenRouterCatalog` is a single
// `SELECT sync_openrouter_catalog($1, $2)`.
//
// Three judgements live here, and each one is a decision somebody could
// reasonably make differently:
//
//   Price.    OpenRouter quotes USD *per token*, as decimal strings
 ...
 * produce text out. Skipping is not silence — `syncOpenRouterCatalog` returns
 * both counts, and the Settings screen renders the one it wrote.
 */
export function normaliseModels(body: unknown): SyncRow[] {
  ...
  for (const raw of data as OpenRouterModel[]) {
    ...
    const modelId = openRouterCatalogId(raw.id);
    ...
    out.push({
      ...
      effort_map: supportsReasoning ? { ...OPENROUTER_EFFORT_MAP } : null,
      default_effort: supportsReasoning ? OPENROUTER_DEFAULT_EFFORT : null,
      pricing_per_million: {
        input,
        output,
        input_off_peak: null,
        output_off_peak: null,
        // OpenRouter prices a cache read separately when the upstream does.
 ...
 * other than `openrouter` or touch a row whose `source` is `seed`.
 */
export async function syncOpenRouterCatalog(tx: Tx, body: unknown, today: Date = new Date()): Promise<SyncResult> {
  const rows = normaliseModels(body);
