...
export function runtimeSkillCards(env: Env, agentId: string): readonly [{
  ...
  return [{
    ...
    detail: partnerAgentConfig(env, agentId).config?.source === 'agentcash_people'
      ? 'AgentCash People Search is attached through one exact, leased $0.15 call. Explicit requests may run a separate $0.01 LinkedIn/YouTube creator search or $0.005 X public-post search. One shortlisted candidate may receive $0.05 professional contact enrichment and $0.03 email verification. Iris prepares a draft-only review; no message, call, or text is sent.'
