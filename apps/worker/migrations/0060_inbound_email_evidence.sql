...
-- A manual external-completion receipt is evidence, not provider execution.
-- It does not mutate effects.status and is not available for payments/email.
CREATE TABLE IF NOT EXISTS external_effect_evidence_receipts (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  workspace_id        uuid NOT NULL REFERENCES workspaces (id) ON DELETE CASCADE,
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    ...
    'external_effect_evidence_receipts'
  ] LOOP
...
GRANT SELECT, INSERT ON mailbox_thread_snapshots, inbound_email_events,
  external_effect_evidence_receipts TO app;
...
REVOKE UPDATE, DELETE ON mailbox_thread_snapshots, inbound_email_events,
  external_effect_evidence_receipts FROM app;
REVOKE ALL ON gmail_evidence_accounts, gmail_evidence_oauth_states,
  mailbox_thread_snapshots, inbound_email_events,
  external_effect_evidence_receipts FROM agent;
...
CREATE TRIGGER inbound_email_events_append_only BEFORE UPDATE OR DELETE ON inbound_email_events
  FOR EACH ROW EXECUTE FUNCTION refuse_mutation();
DROP TRIGGER IF EXISTS external_effect_evidence_receipts_append_only ON external_effect_evidence_receipts;
CREATE TRIGGER external_effect_evidence_receipts_append_only BEFORE UPDATE OR DELETE ON external_effect_evidence_receipts
  FOR EACH ROW EXECUTE FUNCTION refuse_mutation();
