...
The default dedicated profile is `~/.hermes-enterprise/<agent-uuid>/`, separate from the user's personal `~/.hermes`. It contains its own `home`, `os-home`, working directory, session database, run reservations, API key, and a copy of the plugin. `--state-root` can move this data; keep the path short enough for macOS Unix sockets. No personal environment, bot token, provider key, proxy setting, `HERMES_HOME`, session identity or Python user path is inherited. The launcher takes an exclusive per-agent file lock and refuses to repurpose an existing profile for a different workspace/agent/enterprise URL.
