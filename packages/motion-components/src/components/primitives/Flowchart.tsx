 ...
 * New application, cited screening and human review,
...
export const PARTNER_STEPS: StepNode[] = [
...
export type FlowEdge = { from: string; to: string; loop?: boolean };
const EDGES: FlowEdge[] = [{ from: "trigger", to: "screen" }, { from: "screen", to: "review" }, { from: "review", to: "trigger", loop: true }];
const EST_H: Record<string, number> = { trigger: 92, screen: 92, review: 134 };
const PROPERTIES = ["status", "reviewer", "role", "evidence"];
const STATUSES = [{ name: "Needs review" }, { name: "Context missing" }, { name: "Reviewed" }];
const REVIEWERS = [{ name: "Maya Chen" }, { name: "Dana Park" }];
...
function PartnerIcon({ size = 16 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <rect x="4" y="5" width="16" height="15" rx="3" /><path d="M8 3v4M16 3v4M8 11h8M8 15h5" />
    </svg>
  );
}
...
function ConditionBody({ onChange }: { onChange?: (values: FlowConditions) => void }) {
    ...
    <div className="flex flex-col gap-1.5 px-3 py-2.5">
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1.5">
        ...
        <span className="max-w-full pl-[49px]">
          {chip("val2", REVIEWERS, "w-64", { dot: true })}
        </span>
      </div>
    </div>
  );
...
/* ── the canvas ── */
export default function Flowchart({ steps = PARTNER_STEPS, edges = EDGES, onSelect, onMove, onConditionsChange }: { steps?: StepNode[]; edges?: FlowEdge[]; variant?: string; onSelect?: (step: StepNode | null) => void; onMove?: (id: string, offset: { dx: number; dy: number }) => void; onConditionsChange?: (values: FlowConditions) => void } = {}) {
  ...
  const bezier = (edge: FlowEdge) => {
    ...
    if (edge.loop) { const side = Math.max(12, cw * 0.06); return `M ${from.x} ${from.y} C ${from.x} ${from.y + 30}, ${side} ${from.y + 30}, ${side} ${from.y} L ${side} ${to.y - 12} Q ${side} ${to.y - 30}, ${to.x} ${to.y}`; }
  ...
  const onPointerUp = (node: StepNode) => () => {
    ...
    if (d?.id === node.id) {
      ...
      if (d.moved) { onMove?.(node.id, offsets[node.id]); setTimeout(() => (drag.current = null), 0); }
          ...
          >
            ...
            {node.condition ? (
              <div className="w-full rounded-[18px] bg-surface shadow-card transition-shadow duration-150 hover:shadow-raised">
                <ConditionBody onChange={onConditionsChange} />
              </div>
            ) : (
              <button
                type="button"
                onClick={() => {
                  ...
                  setSelected(active ? null : node.id); onSelect?.(active ? null : node);
                }}
                aria-pressed={active}
