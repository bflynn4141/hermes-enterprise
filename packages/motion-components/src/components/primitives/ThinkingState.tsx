 ...
 *   Reasoning  observable findings that expand, then settle
...
function useSequence(steps: number[], reducedMotion: boolean) {
...
export default function ThinkingState({
  ...
  const stage = controlledStage === undefined ? automaticStage : Math.max(0, Math.min(4, controlledStage));
  ...
  useEffect(() => {
    if (working) { settledRef.current = false; return; }
              ...
              const content = (
                ...
                {variant === "Steps" && (
                  i < visible - 1 || !working ? (
                    ...
                    <span className="size-3 shrink-0 rounded-full border-[1.5px] border-line-strong border-t-ink-2" style={{ animation: reducedMotion ? "none" : "spin 700ms linear infinite" }} />
                  )
                )}
