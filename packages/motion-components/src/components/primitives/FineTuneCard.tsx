...
const OPTIONS = ["Context", "Review", "Result"];
...
export default function FineTuneCard({
  ...
  useEffect(() => {
    if (!menuOpen) return;
    const pointer = (event: PointerEvent) => { if (!menuRef.current?.contains(event.target as Node)) setMenuOpen(false); };
    const key = (event: KeyboardEvent) => { if (event.key === "Escape") { setMenuOpen(false); menuButtonRef.current?.focus(); } };
      ...
      <div className="primitive-card-bar flex items-center justify-between border-b border-line">
        ...
        {done ? (
          ...
          <span className="flex items-center gap-1.5">
            <span className="flex size-4.5 items-center justify-center rounded-[5px] border border-accent-ink/30 bg-accent-tint">
              <svg width="9" height="9" viewBox="0 0 24 24" fill="var(--accent-ink)">
            ...
            <span
              ...
              style={{
                backgroundImage:
                  "linear-gradient(90deg, var(--accent-ink) 35%, var(--ink) 50%, var(--accent-ink) 65%)",
