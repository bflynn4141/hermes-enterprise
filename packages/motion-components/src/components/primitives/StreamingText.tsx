...
const TOKENS: StreamingToken[] = [
  ..."Leah and Owen are ready for your review. Both included integration examples that match the proposed partner criteria."
    .split(" ")
    .map((text) => ({ text })),
  { text: "", cite: true },
  ..."I saved the evidence with each application. Admission and access still need human approval."
    .split(" ")
    .map((text) => ({ text })),
];
...
export default function StreamingText({
  ...
  useEffect(() => { setCount(0); setSourcesOpen(false); doneRef.current = false; }, [content]);
  ...
  const act = async (index: number) => {
    const action = (["copy", "retry", "helpful", "collective"] as const)[index];
    if (action === "copy") {
      try {
        await navigator.clipboard.writeText(content.filter((token) => !token.cite).map((token) => token.text).join(" "));
        setActionFeedback("Copied");
      } catch { setActionFeedback("Couldn’t copy. Select the response to copy it."); }
    } else if (action === "retry") {
      setCount(0); setSourcesOpen(false); doneRef.current = false;
  ...
  return (
    <div data-reduced-motion={reducedMotion || undefined} className={fill ? "w-full" : "min-h-[15.5rem] w-full max-w-95"}>
      <p className="text-[13px] leading-relaxed text-ink">
        {content.slice(0, reducedMotion ? content.length : count).map((token, i) =>
          token.cite ? (
        ...
        {ACTION_ICONS.map((icon, i) => (
          ...
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              {i === 3 ? <g><path d="M12 3 3 8v9l9 5 9-5V8zM3 8l9 5 9-5M12 13v9" /><path d="M16 3v6M13 6h6" /></g> : icon}
            </svg>
          </button>
        ))}
        <button
      ...
      <span role="status" className="sr-only">{actionFeedback}</span>
