...
function RollingDigits({ value }: { value: string }) {
  ...
  useEffect(() => {
    ...
    if (reduce) { prevRef.current = value; setOldVal(value); setNewVal(value); setRolling(false); return; }
...
export default function ApprovalCard({
  ...
  const previousQuestionRef = useRef(qi);
  ...
  // A selected radio can advance while it still owns focus. Move focus into
  // the newly visible panel immediately; the visual 360ms slide is independent.
  useLayoutEffect(() => {
    if (previousQuestionRef.current === qi) return;
    previousQuestionRef.current = qi;
    questionRefs.current[qi]?.querySelector<HTMLElement>("[data-question-heading]")?.focus({ preventScroll: true });
  }, [qi]);
    ...
    <div className="w-full max-w-80">
      <div className="relative overflow-hidden rounded-card bg-surface shadow-card" style={{ animation: "fade-up 380ms cubic-bezier(0.23,1,0.32,1) both" }}>
        <button
          type="button"
          aria-label="Dismiss"
          onClick={() => { if (advanceTimer.current) clearTimeout(advanceTimer.current); setOpen(false); }}
              ...
              {questions.map((question, qIdx) => {
                ...
                return (
                  <div
                    ...
                    style={questionStyle}
                  >
                    <div data-question-heading role="heading" aria-level={3} tabIndex={-1} className="pr-7 text-[14px] font-medium text-ink">{question.q}</div>
        ...
        <div className="primitive-card-footer flex items-center justify-between gap-3">
          ...
          <div className="-mr-0.5 flex items-center gap-1.5">
            <Button variant="ghost" size="sm" onClick={() => { if (advanceTimer.current) clearTimeout(advanceTimer.current); if (last) setOpen(false); else goTo(qi + 1); }}>
              {t.skip}
            </Button>
